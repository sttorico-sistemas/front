...
<template>
  <div v-if="layout === 'row'" :class="classWrapper">
    ...
    <select :id="id" class="form-select h-[38px] flex-1" :class="[classSelect, classDisabled]" :required="required"
      :disabled="disabled">
      <option value="">{{ placeholder }}</option>
      <option v-for="option in options" :value="value">{{ option }}</option>
    </select>
  </div>

  <div v-if="layout === 'col'" class="flex sm:flex-row flex-col items-center mb-3">
    <label :for="id" class="mb-0 md:w-1/5 mr-2 w-full text-left" :class="classLabel">{{ label
      }}</label>
    <select :id="id" class="form-select h-[38px] flex-1" :class="[classSelect, classDisabled]" :required="required"
      :disabled="disabled">
      <option value="">{{ placeholder }}</option>
