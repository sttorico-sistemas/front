<script lang="ts" setup>
  ...
  import CadastrarConsignacao from '../modal-cadastrar-consignacao/modal-cadastrar-consignacao.vue'
...
<template>
	<main>
        ...
        <div class="flex items-center gap-14">
					...
					<tippy target="right" placement="right"
						>Cadastre uma nova consignação</tippy
					>
				</div>
        ...
        <div
				...
				>
					<multiselect
						v-model="tipoServico"
						:options="['Empréstimos', 'Cartão Crédito']"
          ...
          <multiselect
            v-model="classe"
            :options="['Facultativa', 'Facultativa']"
          ...
          />
          <multiselect
            v-model="tipoConsignacao"
            :options="['Amortização', 'Pagamentos']"
            class="custom-multiselect min-w-[170px]"
            placeholder="Tipo Consignação"
            :searchable="false"
            :preselect-first="false"
            :allow-empty="false"
            selected-label=""
          ...
          />
					...
					<div class="w-5 h-5">
						<consultas-export
							...
							:cols="parseCols()"
							:rows="rows"
							filename="Normativos Cadastrados"
							export-type="print"
						>
							<template #icon>
          ...
          <template #actions="data">
              ...
              <div>
								...
								<tippy target="right" placement="right"
									>{{ data.value.status === 'Ativo' ? 'Inativo' : 'Ativo' }}</tippy
								>
							</div>
            </div>
					</template>
    ...
    <modal-layout
      title="Cadastrar Nova Consignação"
