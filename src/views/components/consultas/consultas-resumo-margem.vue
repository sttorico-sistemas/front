	...
	<main>
		...
		<div class="panel mt-6">
			<div class="flex flex-wrap justify-between md:items-center md:flex-row flex-col mb-5 gap-5">
				<consultas-titulo title="Resumo da Margem Consignável"
					...
					<template #uso_margem="data">
						<div class="flex justify-between items-center gap-3">
							<div class="w-10/12 rounded-full h-2 bg-dark-light shadow">
								<div class="h-full rounded-full" :class="color(data.value.id)"
									:style="`width: ${data.value.uso_margem}%`" />
							</div>
		...
		<div class="panel pb-0 mt-6">
			<div class="flex flex-wrap justify-between md:items-center md:flex-row flex-col mb-3 gap-5">
				<consultas-titulo title="Saldo Disponível da Margem"
					info-message="Alguma mensagem aqui para tirar dúvida sobre a página" />
			</div>

			<div id="saldo_disponivel" class="flex justify-between items-center py-8">
				<multiselect 
					v-model="servico" 
					:options="optionsServicos" 
					class="custom-multiselect w-[286px] h-[48px]" 
					:searchable="false"
				...
				<button
					type="button"
					class="w-[77px] h-[48px] btn border-primary_3-table text-primary_3-table shadow-none"
					@click="clear"
				>
					Limpar
				</button>

				<div class="w-[575px] h-[48px] flex relative items-stretch flex-wrap">
					<div class="ltr:-mr-px rtl:-ml-px flex">
						<span
							class="border border-[#e0e6ed] rounded-l bg-[#EAEAEC] flex items-center justify-center text-black px-4 py-1.5 ">
							<calculatorIcon />
						</span>
					</div>
					<span class="flex-1 flex items-center border rounded-r px-2 calcula_saldo">Calcule o saldo disponível da sua
						Margem</span>
				</div>
			</div>
		</div>

		<modal-layout :is-open="isOpenDialog" title="Extrato Mensal dos Descontos" size="max-w-full" btn-close
