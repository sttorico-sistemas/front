	...
	<div class="flex flex-col">
		<div class="border border-primary rounded-md p-5">
			...
			<form-field v-slot="{ componentField }" name="cpf">
				<form-item class="grid grid-cols-24 items-center gap-x-4 gap-y-1">
					<form-label class="text-left col-span-5">CPF</form-label>
					<form-control>
						<input-root
							v-maska="'###.###.###-##'"
							:disabled="disabled"
			...
			<form-field v-slot="{ componentField }" name="birthday">
				<form-item class="grid grid-cols-24 items-center gap-x-4 gap-y-1">
					<form-label class="text-left col-span-5">Data de nascimento</form-label>
					<form-control>
						<input-root
							v-maska="'##/##/####'"
							:disabled="disabled"
				...
				<form-item class="grid grid-cols-12 items-center gap-x-4 gap-y-1">
					<form-root
						...
						@submit="
							(values, context) => {
								const isInvalid = (
									(metadata?.contacts ?? []) as Array<any>
								).some(
								...
								if (isInvalid) {
									contactsSetErrors(
										'Não é permitido adicionar contatos iguais.',
									)
									return
								}
									...
									<popover v-model:open="openCityBox">
										...
										<popover-content class="lg:max-w-96 flex-[3]A p-0">
											<command>
												...
												/>
												<command-empty
													>Nenhuma cidades encontrada.</command-empty
												>
												<command-list>
													<command-group>
														<command-item
