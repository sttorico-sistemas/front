...
const nome = ref('');
const form = ref({
  tipo_operador_id: null, // Inicializa vazio
});
const tiposOperadores = ref([]);
const allPermissions = ref([]); // Todas as permissões disponíveis
const selectedPermissions = ref([]); // Permissões selecionadas
...
// Buscar pessoa por CPF (somente no modo de criação)
const searchCPF = async () => {
  if (!cpf.value) return;

  try {
    const response = await axios.get(
      `https://dev-02-apiv2.management.infoconsig.tec.br/api/pessoa/search?cpf=${cpf.value}`
    );
    if (response.data.length > 0) {
      const pessoa = response.data[0];
      nome.value = pessoa.nome || 'Nome não encontrado';
    } else {
      nome.value = 'Nome não encontrado';
    }
  } catch (error) {
    console.error('Erro ao buscar pessoa por CPF:', error);
    nome.value = 'Erro ao buscar nome';
  }
};
...
watch(
  ...
  { immediate: true } // Executar ao montar o componente
);

// Atualizar permissões ao trocar tipo de operador
watch(
  () => form.value.tipo_operador_id,
  (newTipoId) => {
    const tipoOperador = tiposOperadores.value.find((tipo) => tipo.id === newTipoId);
    selectedPermissions.value = tipoOperador ? tipoOperador.permissao_ids : [];
  }
);
