...
import { tabelasAuxiliaresStore as _tabelasAuxiliaresStore } from '@/modules/configuracoes/stores'
...
export const pessoaStore = defineStore('pessoaStore', {
	...
	actions: {
		...
		async toggleStatus(pessoa: Pessoa) {
			...
			this.updateEditingPessoa({
				status: pessoa.status === 'Ativado' ? 'Inativado' : 'Ativado',
			})
		...
		toggleDeletePessoa(props?: { pessoa?: Pessoa; show?: boolean }) {
			if (!props?.pessoa) {
	...
	getters: {
		editingAddresses(): {
			...
			return (
				this.pessoas
					.find((e) => e.id == this.editingPessoa.id)
					?.enderecos.map((e) => ({
						cidade: e.cidade.nome,
						endereco: e.logradouro,
						loadingCidades: false,
						tipoEndereco:
							this.tipoEnderecos.find(
