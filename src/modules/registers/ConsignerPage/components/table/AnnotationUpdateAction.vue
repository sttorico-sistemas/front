<script lang="ts" setup>
	...
	import { annotationRepository } from '@/core/stores'
	import AnnotationForm from './AnnotationForm.vue'
	import { AnnotationModel } from '@/core/models'

	const properties = defineProps({
		dataId: { type: Number, required: true },
		tableAnnotationName: { type: String, required: true },
	...
	async function setNewData() {
		...
		try {
			const data = await annotationRepository.getAnnotationById(
				properties.dataId,
			)
...
<template>
	<form-wrapper
		...
		:title="`Editar averbador ${tableAnnotationName}`"
