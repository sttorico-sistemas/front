...
const selectUf = useRouteQuery<string | undefined>('city-uf-select', undefined)
...
const {
	...
	isFetching: isSelectCityFetching,
	refetch: selectCityRefetch
} = useQuery({
	queryKey: tabelasAuxiliaresRepository.getQueryKey('cities', { page: page.value, limit: perPage.value }),
	queryFn: ({ signal }) => tabelasAuxiliaresRepository.getAllTableValues('auxiliary/cidade', {
...
const { mutateAsync: handleDeleteCity, isPending: isDeleteCityLoading } = useMutation({
	...
	onSettled: async () => {
		return await queryClient.invalidateQueries({
			queryKey: tabelasAuxiliaresRepository.getQueryKey('auxiliary/cidade', { page: page.value, limit: perPage.value })
		})
	},
...
const { mutateAsync: handleUpdateCity, isPending: isUpdateCityLoading } = useMutation({
...
const { mutateAsync: handleCreateCity, isPending: isCreateCityLoading } = useMutation({
...
const formattedAllCities = computed(() => {
	return (selectCityData.value ?? []).map(({ id, nome }) => ({ id, name: nome }))
})
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			return h(
				...
				[
					h(EditTableCity, { dataId: data.id, tableCityName: data.name, "onOn-edit": onUpdateSubmit, isLoading: isUpdateCityLoading.value }),
					h(DeleteTableCity, { dataId: data.id, tableCityName: data.name, "onOn-delete": onDeleteSubimit, isLoading: isDeleteCityLoading.value })
				]
			)
		},
	}
...
const table = useVueTable({
	get data() { return formattedAllCities.value },
	...
	<div class="panel mt-6">
		<div class="flex flex-wrap justify-between md:items-center md:flex-row flex-col mb-5 gap-5">
			<div class="flex gap-10 items-center justify-center">
				...
				<form-wrapper v-model="openCreateModal" :is-loading="isCreateCityLoading" :title="`Criar nova cidade`"
					description="Crie o conteúdo de uma nova cidade." class="sm:max-w-[780px]" @form-submit="onCreateSubmit">
					...
					<template #fields>
						<TableCityForm :metadata="form.values" :disabled="isCreateCityLoading" />
					</template>
				</form-wrapper>
			</div>

			<div class="header_actions flex items-center gap-4 flex-1 justify-end">
				<select-root class="flex-[1]" :disabled="formattedAllStates.length <= 0" v-model:model-value="selectUf"
					@update:model-value="handleSelectCity">
					<select-trigger class="lg:max-w-96 flex-[2]">
						<select-value class="text-left" placeholder="Selecione um estado..." />
					</select-trigger>
					<select-content>
						<select-group>
							<select-label>Estados:</select-label>
							<select-item v-for="state of formattedAllStates" :key="state.id" :value="state.id.toString()">{{
								state.name }}</select-item>
						</select-group>
					</select-content>
				</select-root>

				<select-root :disabled="formattedAllCities.length <= 0" v-model:model-value="selectCity"
					@update:model-value="handleSelectCity">
					<select-trigger class="lg:max-w-96 flex-[3]">
						<select-value class="text-left" placeholder="Selecione uma cidade..." />
					</select-trigger>
					<select-content>
						<select-group>
							<select-label>Cidades:</select-label>
							<select-item v-for="city of formattedAllCities" :key="city.id" :value="city.id.toString()">{{
		...
		<div class="datatable pb-1">
			<table-wrapper :table="table" :column-size="columns.length" :row-limit="perPage"
				:is-loading="isSelectCityPending || isSelectCityFetching" />
			...
			<div :class="['flex w-full items-center px-4']">
				...
				<TablePagination v-model="page" :disabled="formattedAllCities.length <= 0"
